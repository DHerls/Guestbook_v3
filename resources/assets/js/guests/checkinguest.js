...
const app = new Vue ({
    ...
    methods: {
        sig_tablet: function(type) {
            this.tablet_target = type
            $('#sigmodal').modal('show');

            var ctx = document.getElementById('tabletsig').getContext('2d');
            SetDisplayXSize( 600 );
            SetDisplayYSize( 200 );
            SetTabletState(0, this.tablet);
            SetJustifyMode(0);
            ClearTablet();
            if(this.tablet == null)
            {
                this.tablet = SetTabletState(1, ctx, 50);
            }
            else
            {
                SetTabletState(0, this.tablet);
        ...
        sig_done: function() {
            ...
            SetImageYSize(200);
            SetImagePenWidth(5);
            GetSigImageB64(function(string) {
                if (app.tablet_target == "guest"){
                    guestSig.fromDataURL("data:image/png;base64,"+string);
                } else {
                    memberSig.fromDataURL("data:image/png;base64,"+string);
                }
                $('#sigmodal').modal('hide');
            });
        },
